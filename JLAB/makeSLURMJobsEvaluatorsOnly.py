...
class pars:
  ...
  randomTimeDelay   = '10'        # set to 0 for no random delay. Otherwise it will be between 0 and this many seconds
  eval_revision     = 2           # Force the evaluator revision for output. Set to "-1" to use automatic default
...
def makeSLURMJob():
    ...
    while line:
       ...
       for i in range(nJobsFromFile):
            ...
            slurmFile.write("#SBATCH --time=12:00:00\n")
            slurmFile.write("#SBATCH --gres=disk:10000\n")
            slurmFile.write("#SBATCH --output=" + slurmOutputInfo + "-eval_only.out\n")
            slurmFile.write("#SBATCH --error=" + slurmOutputInfo + "-eval_only.err\n")
            ...
            slurmFile.write("SCRIPT=./run_EIC_evaluators.sh\n")
            slurmFile.write("\n")
            ...
            slurmFile.write("\n")
            slurmFile.write("# Originally we created a dedicated working directroy on /u/scratch\n")
            slurmFile.write("# This had the benefit of persisting the working directory in an\n")
            slurmFile.write("# accessible location after the job ended. It also had the potential to\n")
            slurmFile.write("# cause issues if the remote scratch disk was not responsive. Thus,\n")
            slurmFile.write("# we use the dedicated local directory automatically created by slurm.\n")
            slurmFile.write("#\n")
            slurmFile.write("# mkdir -p ecce_work/workdir_${SLURM_JOBID}\n")
            slurmFile.write("# cd ecce_work/workdir_${SLURM_JOBID}\n")
            slurmFile.write("cd /scratch/slurm/$SLURM_JOB_ID\n")
            slurmFile.write("printf \"cwd: \"; /bin/pwd\n")
            slurmFile.write("\n")
            ...
            slurmFile.write("echo \n")
            slurmFile.write("echo \"----------------------------------------------\"\n")
            slurmFile.write("# (see comment above on use of /u/scratch)\n")
            slurmFile.write("#cd ../../../../\n")
